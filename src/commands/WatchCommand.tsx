	...
	switch (action.type) {
		...
		case "ERROR":
			return {
				...
				items: state.items.map((i) =>
					i.tweetId === action.tweetId
						? { ...i, phase: "error", error: action.message }
						: i,
				),
			};
		default:
...
export const WatchCommand: React.FC<Props> = ({
	outputDir,
	quality,
	sendNotify,
}) => {
			...
			if (tweet.videoVariants.length > 0) {
				...
				if (sendNotify)
					notifyDownloadDone(
						...
						`${tweet.photos.length} photo${tweet.photos.length > 1 ? "s" : ""}`,
					);
			} else {
			...
			{active && (
				<Box flexDirection="column" marginBottom={1}>
					...
					{!active.progress && active.phase === "fetching" && (
						<Box marginLeft={2}>
							<Text color="#555555">Fetching tweet…</Text>
						</Box>
					)}
