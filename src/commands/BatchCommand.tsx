...
function reducer(state: State, action: Action): State {
	...
	switch (action.type) {
		...
		case "FETCHING":
			return update(action.tweetId, { phase: "fetching" });
		case "DOWNLOADING":
			return update(action.tweetId, {
				phase: "downloading",
				username: action.username,
				quality: action.quality,
			});
		case "PROGRESS":
			return update(action.tweetId, { progress: action.progress });
		case "DONE":
			return update(action.tweetId, {
				phase: "done",
		...
		case "SKIP":
			return update(action.tweetId, { phase: "skip", error: action.reason });
		case "ERROR":
			return update(action.tweetId, { phase: "error", error: action.message });
		default:
			return state;
	}
}
...
export const BatchCommand: React.FC<Props> = ({
	...
	const [totalCount] = React.useState(() => {
		try {
			return parseUrlsFromFile(batchFile).length;
		} catch {
			return 0;
		}
	});
	...
	useEffect(() => {
				...
				try {
					...
					if (tweet.videoVariants.length > 0) {
						...
						const filePath = await downloadVideo(
							...
							filename,
							(p) =>
								dispatchRef.current({ type: "PROGRESS", tweetId, progress: p }),
						);
		...
		Promise.all(tasks).then(() => {
			const _doneItems = items.filter((_) => {
	...
	}, [quality, concurrent, sendNotify, outputDir, exit, batchFile]);
	...
	const active = items.filter(
		(i) => i.phase === "downloading" || i.phase === "fetching",
	).length;
	...
	const visible = [
		...
		...items.filter(
			(i) => i.phase === "done" || i.phase === "skip" || i.phase === "error",
		),
	].slice(0, 18);
		...
		<Box flexDirection="column" paddingLeft={2}>
			...
			{visible.map((item) => {
				...
				const label = item.username
					? `@${item.username.padEnd(14)}`
					: item.tweetId.slice(-8).padEnd(14);
